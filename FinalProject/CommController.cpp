...
void CommController::readHandle(DWORD bytesToReceive){
	...
	if (ReadFile(commHandle, inputBuffer, bytesToReceive, &bytesReceived, &overlapRead)) {
		...
		int  overlapped = GetOverlappedResult(commHandle, &overlapRead, &bytesReceived, TRUE);

		//ERROR_IO_PENDING is not being signalled 
		//last error to happen is ERROR_SUCCESS
